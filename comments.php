...
<section id="respond" class="respond-form">
	...
	<h3 id="comment-form-title" class="h2"><?php comment_form_title( __('Get involved:', 'bonestheme'), __('Holla back at %s', 'bonestheme' )); ?></h3>
	...
	<?php if ( get_option('comment_registration') && !is_user_logged_in() ) : ?>
		<div class="alert alert-help">
			<p><?php printf( __('Dude you must be %1$slogged in%2$s to post a comment.', 'bonestheme'), '<a href="<?php echo wp_login_url( get_permalink() ); ?>">', '</a>' ); ?></p>
		</div>
	<?php else : ?>
	...
	<p class="comments-logged-in-as"><?php _e("Logged in as", "bonestheme"); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e("Log out of this account, bro?", "bonestheme"); ?>"><?php _e("Log out bro", "bonestheme"); ?> <?php _e("&raquo;", "bonestheme"); ?></a></p>
	...
	<ul id="comment-form-elements" class="clearfix">
		...
		<li>
			...
			<input type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" placeholder="<?php _e('Your name, handle, etc', 'bonestheme'); ?>" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> />
		</li>
	...
	<p><textarea name="comment" id="comment" placeholder="<?php _e('Type your Electronic Internet Website Cyber-Comment here...', 'bonestheme'); ?>" tabindex="4"></textarea></p>

	<p>
		<input name="submit" type="submit" id="submit" class="button" tabindex="5" value="<?php _e('Post this comment', 'bonestheme'); ?>" />
